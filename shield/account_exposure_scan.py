...
def _services(results, *, attempted, checked, status):
    probable = [
        {
            "service_name": name,
            "status": "probable",
    ...
    return {
        ...
        "not_found_count": 0,
        "not_checked_count": 0 if status == "completed" else 1,
        "results": probable,
    }


def _base(email, status, *, error_reason=None, services=None, source_status=None, checked=0, exit_code=None):
    services = services or []
    source_status = source_status or status
    attempted = 1 if source_status in {"completed", "error", "timeout"} else 0
    source = source_record(
        SOURCE_ID,
        source_status,
        confidence=0.35 if source_status == "completed" else None,
        error_reason=error_reason if source_status != "completed" else None,
        evidence={"checked": checked, "services_count": len(services), "process_exit_code": exit_code},
    )
    return {
        ...
        "services": _services(services, attempted=attempted, checked=checked, status=source_status),
        "sources": [source],
    }
