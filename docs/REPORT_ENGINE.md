...
For `phone_public_web`, JSON/HTML render provider/status and semantic-match-level counts, accepted public URLs and source domains, matched variants, discovered entities, evidence references, source-independence groups, evidence quality, neutral correlations, hypotheses, alternative explanations, and recommended non-executing pivots. `FALSE POSITIVES REJECTED` preserves numeric resource-ID candidates and their rejection reasons without presenting them as public phone occurrences. The section explicitly identifies PASSIVE_WEB exposure and uses “Public occurrence”, “Possible association”, and “Not independently verified”; it never reports an owner or subscriber as identified.
