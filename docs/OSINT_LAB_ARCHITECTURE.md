...
Foundation v2 implements a validated case manifest, fail-closed PolicyGate, plaintext local Evidence Vault, in-memory relation graph and deterministic contradiction checks. It does not implement collection, per-run approval execution, encrypted storage, graph persistence, automatic identity resolution or AI verdicts.
