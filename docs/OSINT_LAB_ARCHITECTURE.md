...
Etap 12 adds the first real phone public-web sensor without bypassing that pipeline. PHONE plans `phone_metadata` first and `phone_public_web` second. The public step is denied before HTTP unless `PASSIVE_WEB` is enabled. Only semantically validated phone-context or structured-telephone occurrences become direct technical evidence; numeric resource identifiers are rejected before correlation. Extracted entities feed neutral correlations and non-executing pivots, and same-domain/content/URL copies are clustered as one independence group. No automatic subscriber or owner conclusion is created.
