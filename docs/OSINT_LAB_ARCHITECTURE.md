...
Etap 15.2 adds a bounded `MultiProviderDiscoveryEngine` in front of the unchanged target-validation path. Brave Search API is credential-gated from process environment and DDG remains an independent fallback. Provider results are candidates only; evidence still begins at a safely fetched and semantically validated target page. Provider/request/query/candidate coverage is visible in the private report and Windows summary.
